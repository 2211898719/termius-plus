...
public class QueryUtils {
     ...
     * @param <E> 关联对象的类型
     */
    public static <T, E> void batchQueryOneToOne(List<E> list,
        ...
        Map<Long, T> collect = joinElements.stream()
                                           .collect(Collectors.toMap(joinFieldGetKeyFun, Function.identity()));
     ...
     * @param <E>                关联对象的类型
     */
    public static <T, E> void batchQueryOneToMany(List<E> list,
                                                  Function<E, Long> getJoinFieldKeyFun,
                                                  Function<List<Long>, List<T>> idInQueryFun,
                                                  Function<T, Long> joinFieldGetKeyFun,
                                                  BiConsumer<E, List<T>> execFun
    ) {

        List<Long> joinIds = list.stream().map(getJoinFieldKeyFun).collect(Collectors.toList());
        List<T> joinElements = idInQueryFun.apply(joinIds);
        Map<Long, List<T>> collect = joinElements.stream()
                                                 .collect(Collectors.groupingBy(joinFieldGetKeyFun));

        list.forEach(element -> {
            List<T> el = collect.get(getJoinFieldKeyFun.apply(element));
            if (el == null) {
                return;
            }
