...
public class EncryptionFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        boolean decryptionStatus = this.contains(
                encryptionConfig.getRequestDecryptUriList(),
                uri,
                req.getMethod(),
                req
                                                );
        boolean encryptionStatus = this.contains(
                encryptionConfig.getResponseEncryptUriList(),
                uri,
                req.getMethod(),
                req
                                                );
        boolean decryptionIgnoreStatus = this.contains(
                encryptionConfig.getRequestDecryptUriIgnoreList(),
                uri,
                req.getMethod(),
                req
                                                      );
        boolean encryptionIgnoreStatus = this.contains(
                encryptionConfig.getResponseEncryptUriIgnoreList(),
                uri,
                req.getMethod(),
                req
                                                      );
    ...
    private void processDecryption(EncryptionReqestWrapper requestWrapper, HttpServletRequest req) {
        ...
        try {
            ...
            String prefixUri = req.getMethod()
                                  .toLowerCase() + ":" + uri;
            while (parameterNames.hasMoreElements()) {
                String paramName = parameterNames.nextElement();
                String paramValue = req.getParameter(paramName);
                if (encryptionConfig.getRequestDecryptParams(prefixUri)
                                    .contains(paramName)) {
    ...
    private void writeEncryptContent(String responseData, ServletResponse response) throws IOException {
        ...
        try {
            ...
            Charset charset = Charset.forName(encryptionConfig.getResponseCharset());
    ...
    private boolean contains(List<String> list, String uri, String methodType, HttpServletRequest request) {
        ...
        try {
            ...
            if (Objects.nonNull(handler.getHandler()) && handler.getHandler() instanceof HandlerMethod) {
                ...
                String apiUri = RequestUriUtils.getApiUri(
