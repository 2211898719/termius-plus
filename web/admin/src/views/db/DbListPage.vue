...
const getDbConnList = async () => {
  dbConnList.value.splice(0)
  dbConnList.value.push(...await dbApi.list())
}

getDbConnList()

const getServerList = async () => {
  let list = await dbApi.findAllDbServer()
  data.value.splice(0)
      ...
      <a-space direction="vertical" size="middle" style="width: 100%;">
        <a-card>
          ...
          <div class="mt30 server">
            ...
            <a-list :grid="{ gutter: 16, column: props.column }" :data-source="dbConnList" row-key="id">
              <template #renderItem="{ item }">
                <a-dropdown :trigger="['contextmenu']">
                  <a-list-item class="sortEl" @dblclick="handleDblclick(item)">
                    <template #actions>
                    ...
                    <a-card>
                      <a-skeleton avatar :title="false" :loading="!!item.loading" active>
                        <a-list-item-meta
                            :description="item.host+', '+item.port+', '+item.username"
                        >
                          <template #title>
                            <span>{{ item.connName }}</span>
                          </template>
                          <template #avatar>
                            <DatabaseOutlined style="color: #E45F2B;" class="icon-server"></DatabaseOutlined>
                          </template>
                        </a-list-item-meta>
                      </a-skeleton>
                    </a-card>
